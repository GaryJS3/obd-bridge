...
Live polling starts when OBD control is acquired; it can be paused to send manual commands. Polling restores the standard ECU header when stopped. Charging is detected from charger input current; driving is detected from vehicle speed or HV battery current. The default charging cards use live-verified module `7E4` PIDs for AC current/voltage (`224369`/`224368`) and HV battery current/voltage (`22436C`/`22436B`). SOC uses the provisional `7E4 / 22432F` PID and should be checked against the car display. The remaining EV miles card is an estimate calculated from SOC and the configured full-charge range, which defaults to the 2017 Volt's EPA-rated 53 miles. The actual range changes with driving, temperature, battery condition, and HVAC use.
